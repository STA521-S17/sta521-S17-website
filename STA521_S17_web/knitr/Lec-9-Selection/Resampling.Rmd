...
* Variance of error:  irreducible error
...
```
using simulation to optain the Predictin Interval
...
##   Prediction interval from last fold  NB model

```{r, echo=F}
df = data.frame(fupacts = hiv.test$fupacts, 
                pred = predict(hiv.train.NB, hiv.test, type="response"), 
                lwr = pi[,1], upr=pi[,2]) 
df = df %>% arrange(pred)
...
```{r pois.PI, echo=F}
df = data.frame(fupacts = hiv.test$fupacts, 
                pred = predict(hiv.train.poi, hiv.test, type="response"), 
                lwr = pi[,1], upr=pi[,2]) 
...
* Can overdispersion be reduced by adding other redictors?
